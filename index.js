...
(function() {
  ...
  var scenes = data.scenes.map(function(data) {
    var useEquirect = !!data.imageUrl;
    var urlPrefix = "tiles";
    var source = useEquirect
      ? Marzipano.ImageUrlSource.fromString(data.imageUrl)
      : Marzipano.ImageUrlSource.fromString(urlPrefix + "/" + data.id + "/{z}/{f}/{y}/{x}.jpg", { cubeMapPreviewUrl: urlPrefix + "/" + data.id + "/preview.jpg" });
    var geometry = useEquirect
      ? new Marzipano.EquirectGeometry([{ width: 8192 }])
  ...
  function preloadAdjacent(sceneObj) {
    if (!sceneObj || !sceneObj.data || !sceneObj.data.linkHotspots) return;
    sceneObj.data.linkHotspots.forEach(function(h) {
      var target = scenesById[h.target];
      if (!target) return;
      if (target.data.imageUrl) {
        preloadImage(target.data.imageUrl);
      } else {
        var previewUrl = 'tiles/' + target.data.id + '/preview.jpg';
  ...
  function switchScene(scene) {
    ...
    scene.scene.switchTo({ transitionDuration: 900 });
    startAutorotate();
  ...
  function updateVariants(scene) {
    ...
    container.innerHTML = '';
    if (scene.data.name === '8TH FLOOR' || scene.data.variantOf === '8TH FLOOR') {
      selector.style.display = 'none';
      return;
    }
    // 416alternative views: off and on
    if (scene.data.id === '1-416second') {
      selector.style.display = 'block';
      var offBtn = document.createElement('button');
      offBtn.textContent = 'Luci off';
      offBtn.className = 'variant-button';
      offBtn.addEventListener('click', function() { switchScene(scenesById['2-416coloroption']); });
      container.appendChild(offBtn);
      return;
    }
    if (scene.data.id === '2-416coloroption') {
      selector.style.display = 'block';
      var onBtn = document.createElement('button');
      onBtn.textContent = 'Luci on';
      onBtn.className = 'variant-button';
      onBtn.addEventListener('click', function() { switchScene(scenesById['1-416second']); });
    ...
    } else if (parentKey === '416') {
      variants = data.scenes.filter(function(d) { return d.variantOf === '416'; }).map(function(d) { return scenesById[d.id]; });
    } else {
    ...
    variants.forEach(function(v) {
      ...
      var label = v.data.name;
      if (v.data.variantOf) {
        var t = label.replace(v.data.variantOf, '').replace('COLORE', '').trim();
        if (t) { label = t; }
      }
  ...
  // Initialize scene based on URL hash (use slug mapping)
  var hash = window.location.hash.substring(1);
  var initialId = slugToId[hash] || hash;
  ...
  window.addEventListener('hashchange', function() {
    var hash = window.location.hash.substring(1);
    var newId = slugToId[hash] || hash;
    var newSceneObj = scenesById[newId] || parentScenes[0];
